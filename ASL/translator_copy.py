...
def withoutSkinSegment():
    ...
    while True:
        ...
        c = cv2.waitKey(1) & 0xff
        if(sentence=="C"):
                engine.say('Burger Ordered')
                st.text("Burger")
        if(sentence=="L"):
                engine.say('Lasagna Ordered')
                st.text('Lasagna')
        if(sentence=="A"):
                engine.say('You are invited to Dev Jams')
        if(sentence=="I"):
                engine.say('Fries Ordered')
                st.text('Fries')
        if(sentence=="H"):
                engine.say('Honey chilli potato Ordered')
                st.text('Honey Chilli')
        if(sentence=="A"):
                engine.say("Pasta Ordered!")
                st.text('Pasta')
        else:
                print("Nothing")
